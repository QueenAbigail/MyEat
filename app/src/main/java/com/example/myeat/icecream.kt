...
class icecream : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var databaseReference = database.getReference().child(auth.currentUser?.uid.toString()).child("icecream")
        val pesan = findViewById<Button>(R.id.button9)
        pesan.setOnClickListener{
            databaseReference.child("nama").setValue("Ice Cream")
            databaseReference.child("jumlah").setValue(counter)
            databaseReference.child("harga").setValue(8000)
            finish()
        }
    }
