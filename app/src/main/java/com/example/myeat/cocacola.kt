...
class cocacola : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var databaseReference = database.getReference().child(auth.currentUser?.uid.toString()).child("cocacola")
        val pesan = findViewById<Button>(R.id.button9)
        pesan.setOnClickListener{
            databaseReference.child("nama").setValue("Coca cola")
            databaseReference.child("jumlah").setValue(counter)
            databaseReference.child("harga").setValue(10000)
            finish()
        }
    }
