...
class kentang : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var databaseReference = database.getReference().child(auth.currentUser?.uid.toString()).child("kentang")
        val pesan = findViewById<Button>(R.id.button9)
        pesan.setOnClickListener{
            databaseReference.child("nama").setValue("Kentang")
            databaseReference.child("jumlah").setValue(counter)
            databaseReference.child("harga").setValue(15000)
            finish()
        }
    }
