...
class telor : AppCompatActivity() {
    private lateinit var database: FirebaseDatabase
    private lateinit var auth: FirebaseAuth
    var counter = 1
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_telor)

        database = FirebaseDatabase.getInstance()
        auth = FirebaseAuth.getInstance()
        Log.d("TAG", "onCreate: "+auth.currentUser?.uid.toString())
        var databaseReference = database.getReference().child(auth.currentUser?.uid.toString()).child("telor")
        val pesan = findViewById<Button>(R.id.button9)
        pesan.setOnClickListener{
            databaseReference.child("nama").setValue("Martabak Telor")
            databaseReference.child("jumlah").setValue(counter)
            databaseReference.child("harga").setValue(32000)
            finish()
        }
    }
