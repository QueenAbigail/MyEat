...
class burger : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var databaseReference = database.getReference().child(auth.currentUser?.uid.toString()).child("burger")
        val pesan = findViewById<Button>(R.id.button9)
        pesan.setOnClickListener{
            databaseReference.child("nama").setValue("Burger")
            databaseReference.child("jumlah").setValue(counter)
            databaseReference.child("harga").setValue(28000)
            finish()
        }
    }
